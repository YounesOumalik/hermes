...
import { ArrowUpRight, Bot, Check, Code2, Plus, Save, Sparkles, Trash2, Wrench, X } from 'lucide-react';
...
export default function AgentsPage() {
  ...
  async function loadAgents() { try { const data = await api.get<{ agents: Agent[] }>('api/agents'); setAgents(data.agents || []); } catch (err) { setError(err instanceof Error ? err.message : 'Impossible de charger les agents.'); } }
  useEffect(() => { loadAgents(); }, []);

  async function createAgent() { if (!name.trim()) return; setSaving(true); setError(''); try { await api.post<Agent>('api/agents', { name: name.trim(), system_prompt: systemPrompt.trim() || 'Tu es un assistant précis et utile.', model: null, tools: [] }); setName(''); setSystemPrompt(''); setOpen(false); await loadAgents(); } catch (err) { setError(err instanceof Error ? err.message : 'Création impossible.'); } finally { setSaving(false); } }
  async function deleteAgent(agentName: string) { try { await api.delete(`api/agents/${encodeURIComponent(agentName)}`); await loadAgents(); } catch (err) { setError(err instanceof Error ? err.message : 'Suppression impossible.'); } }
  ...
  return <div className="page"><header className="page-header"><div><div className="eyebrow"><span className="eyebrow-line" /> CONFIGURATION</div><h1>Vos agents</h1><p className="page-subtitle">Donnez à chaque mission un cerveau, un ton et les bons outils.</p></div><button className="button button-primary" onClick={() => setOpen(true)}><Plus size={16} /> Nouvel agent</button></header>
    ...
    <section className="agent-hero"><div><span className="hero-kicker"><Sparkles size={14} /> Agent builder</span><h2>Des assistants qui savent<br /><span>où aller ensuite.</span></h2><p>Un agent Hermes combine une intention claire, une mémoire de travail et des outils contrôlés.</p></div><div className="agent-stack-art"><span className="stack-card stack-back"><Code2 size={17} /> contexte</span><span className="stack-card stack-middle"><Wrench size={17} /> tools</span><span className="stack-card stack-front"><Bot size={18} /> agent</span></div></section>
    ...
    {open && <div className="modal-backdrop" onClick={() => setOpen(false)}><div className="modal" onClick={(event) => event.stopPropagation()}><div className="modal-heading"><div><div className="eyebrow">NOUVEL AGENT</div><h2>Créer un assistant</h2></div><button className="icon-button" onClick={() => setOpen(false)}><X size={18} /></button></div><label>Nom de l’agent<input value={name} onChange={(event) => setName(event.target.value)} placeholder="Ex. Analyste sécurité" autoFocus /></label><label>Instruction système<textarea value={systemPrompt} onChange={(event) => setSystemPrompt(event.target.value)} placeholder="Décrivez son rôle, son ton et sa méthode…" rows={6} /></label><div className="modal-actions"><button className="button button-secondary" onClick={() => setOpen(false)}>Annuler</button><button className="button button-primary" onClick={createAgent} disabled={saving || !name.trim()}>{saving ? 'Création…' : <><Save size={15} /> Créer l’agent</>}</button></div></div></div>}
  </div>;
}
